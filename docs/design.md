...
### 7.4 PrinterConfig (server-side, not stored in DB)
...
- **Expanded** — shows a file-manager-style document list with folder support.
  Folders can be created, renamed, and deleted. Documents can be dragged between
  folders. A search/filter input sits at the top of the list.
